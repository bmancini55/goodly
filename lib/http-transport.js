...
export class HttpTransport {
   ...
   * @return {[type]}                       [description]
   */
  async requestData({ app, sendDataEvent, correlationId }) {
    let channel = app.channel();
    let appExchange = app.appExchange;
    let headers = {
    ...
    return new Promise((resolve) => {
      ...
      app.emit(sendDataEvent, null, { correlationId, headers });
    });
  }
  ...
  async sendData({ app, msg, value }) {
    ...
    try
    {
      const buffer = JSON.stringify({ data: convertToBuffer(value), correlationId });
